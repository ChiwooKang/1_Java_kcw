...
import edu.kh.poly.ex2.model.vo.KCWCalculator;
...
public class AbstractionService {
	...
	// Animal을 상속 받은 자식 클래스를 이용해 객체 생성
		...
		Fish f1 = new Fish();
		f1.setType("어류"); // 일반적인 상속과 같음
						   // 자식 객체 내부에 부모 부분이 있음을 확인
		f1.breath();
		
		Person p1 = new Person();
		p1.setEatType("포유류"); // 자식 객체에 부모 부분이 있음을 확인
		
		
		// 추상 클래스는 부모 타입의 참조 변수로 사용할 수있다 !
		// -> 다형성의 업 캐스팅
		...
		a1.breath();
		a2.breath();
		// 실행 전에는 Animal의 breath()와 연결 된것으로 보임(정적 바인딩)
		// 실행 중 오버라이딩한 자식의 breath()로 연결됨(동적 바인딩)
		// -> 동적 바인딩에 의해 정상 수행 될 것을 알기때문에 에러가 나지 않는다.
	...
	public void ex2() {
		...
		arr[1] = new HGD(); // 업캐스팅
		// 부모 타입 참조 = 자식 객체;
		// 부모 : KH 인터스페이스 == 인터스페이스도 부모 참조 변수로 사용
	...
	public void ex3() {
		...
		Calculator cal= new KCWCalculator();
