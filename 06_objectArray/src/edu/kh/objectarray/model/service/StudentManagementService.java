...
public class StudentManagementService {
	 ...
	 * @param number
	 * @param name
	 * @return 0또는 1(null 인덱스 없음) / 1(null인 인덱스가 있어서 학생 객체의 주소를 추가함)
	 */
	public int addStudent(int grade, int classRoom, int number, String name) {
		
		// 배열 요소 중 아무곳도 참조하지 않는 (null) 인덱스 찾기
	 ...
	 *  0 :	stdArr[idx] 인덱스가 null인 경우(참조x)
	 *  1 :	정상적으로 수정이 된 경우 
	 *  
	 * 
	 */
	public int updateStudent(int idx, int kor, int eng, int math) {
		...
		if(idx < 0 || idx >= stdArr.length) { // 범위 초과 시
			...
			if(stdArr[idx] == null) { // null을 참조하는 인덱스인 경우
				...
				stdArr[idx].setEng(kor);
				stdArr[idx].setMath(kor);
