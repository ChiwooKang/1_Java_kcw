...
public class WhileExample {
	...
	public void ex2() {
		...
		int input = -1; 
		// whil문 내에서 입력 받은 값을 저장할 변수
		// 입력 받은 값이 0인 경우 라는 조건을 판별하기 위해
		// while 문 보다 앞에 먼저 선언
		
		// 하지만 input 변수 선언 시 초기화가 되어있지 않으면 
		// 조건식 수행을 못하기 때문에
		...
		while(input != 0) {
			...
			// input 변수가 해석되지 않는다. ==변수가 존재하지 않는ㄴ다
	...
	public void ex3() {
		...
		while(input != 9) {
			...
			System.out.println("1. 떡볶이(5000)원");
			System.out.println("2. 김밥(3000)원");
			System.out.println("3. 라면(4000)원");
			System.out.println("4. 돈까스(8000)원");
			...
			if (flag && (input >= 1 && input <= 4)) { //true 인 경우 실행( 첫 주문 X) + 메뉴를 잘 입력 했을 때
				menu += ", ";
			}else { //false인 경우 실행
				flag = true;
			}
