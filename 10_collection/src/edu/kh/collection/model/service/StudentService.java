...
public class StudentService {
	...
	public StudentService() {
		...
		stdList.add(new Student("김길순", 17, "서울시 즁구", 'F', 90));
		stdList.add(new Student("가나다", 19, "서울시 강남구", 'M', 100));
		
		
	}
	
	/** stdList에 학생 정보 추가 메서드 // alrt + shift + j
	 * @param name
	 * @param age
	 * @param address
	 * @param gender
	...
	public boolean addStudent(String name, int age, String address, char gender, int score) {
		...
		// List.add(E e) -> 제네릭<E>에 작서되는 타입으로 모든 E가 변화함
		// test1.add(Object e) -> 타입 제한 X, 모든 참조 변수가 Object 타입
		// test2.add(String e) -> 타입 제한 0, 모든 참조 변수가 String 타입
		// test3.add(Integer e) -> 타엡 제한 O, 모든 참조 변수가 Integer 타입
	...
	public boolean  updateStudent(int idx, String address, int score) {
		...
		// 입력 받은 idxrk stdList에 실제 존재하는 요소의 인덱스 범위를 넘어선 경우
		if ( idx > stdList.size()) {
			return false;
		}else {
