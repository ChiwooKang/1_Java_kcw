...
public class ArrayExample {
	...
	public void ex3() {
		...
		// Arrays.toString(배열명) : 배열에 저장된 모든 값을 한 줄로 출력	
	...
	public void ex6() {
		...
		String[] menuArr = {"김밥+라면", "KFC", "맘스터치", "서브웨이", "백반", "순대국", "곰탕", "파스타",
				            "삼겹살" , "빵"};
	...
	public void ex7() {
		...
		//배열의 첫 번째 인덱스 값을 최고/ 최저의 비교 기준으로 삼음
		
		// breakpoint를 지정한 라인 이전에 멈춘다!
		
		//for 문에 break point를 지정하면 증감식이 해석되기 전에 멈춘다(debug mode)
	...
	public void ex8() {
		...
		boolean flag = true;
		// for문이 종료된 후에됴  true : 존재하지 않는다
		// for문이 중료된 후에 false : 존재한다
		...
		for(int i=0 ; i < arr.length ; i++) {
			...
			if( arr[i] == search) { // all[i] 갑이 입력 받은 값과 같다면
				System.out.println(i + "번쨰 인덱스에 존재합니다.");
				
				flag = false; // flag 값을 변경
				break; // 중복 데이터가 없으면 추가 검색이 필요하지 않음
				}
		}
	...
	public void ex9() {
		...
		// arr변수에 저장된 배열의 시작 주소를 복사하여 copyArr에 대입
		int[] copyArr = arr;
...
//  2.		System.arraycopy(arr, 0 , copyArr, 0, arr.length);
